...
BEGIN
  ...
  FROM projects
  WHERE name = 'Timeline E-KPI'
  ORDER BY id
  LIMIT 1;

  IF project_id_value IS NULL THEN
    RAISE EXCEPTION 'Project Timeline E-KPI tidak ditemukan.';
  END IF;
  ...
  LOOP
    ...
    IF parent_task_id_value IS NULL THEN
      ...
      VALUES (
        ...
        CASE WHEN parent_start_date IS NULL OR parent_end_date IS NULL THEN 0 ELSE 100 END,
        CASE WHEN parent_start_date IS NULL OR parent_end_date IS NULL THEN 'Not Started' ELSE 'Done' END,
        'Medium',
    ...
    ELSE
      ...
      SET
        ...
        progress = CASE WHEN parent_start_date IS NULL OR parent_end_date IS NULL THEN 0 ELSE 100 END,
        status = CASE WHEN parent_start_date IS NULL OR parent_end_date IS NULL THEN 'Not Started' ELSE 'Done' END,
